...
test("should call function : first method", () => {
    ...
    api_1.api.fetchAll = jest.fn(); // on crée notre mock function
    api_1.api.fetchAll.mockResolvedValue(fakeData);
...
test("should call function : theard method", () => {
    const mock = jest.spyOn(api_1.api, "fetchAll");
    const fakeData = [{ name: "tintin" }];
    api_1.api.fetchAll.mockResolvedValue(fakeData);
    const res = app_1.app.fetchUser();
...
test("récupérer les utilisateurs", () => {
    ...
    const x = axios_1.default.get.mockResolvedValue(res);
