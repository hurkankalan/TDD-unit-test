...
describe("mock functions examples with promise", () => {
    test("should be promise resolved", () => {
        ...
        return expect(func.mock.results[0].value).resolves.toBe("Promise is resolved !"); // La promesse est bien résolue et renvoie la value "Promise is resolved !" (ne pas oublier de mettre return car on test une async func !)
    });
});
describe("mock functions with add function", () => {
    test("should be return right callback value", () => {
        const func = jest.fn();
        (0, mock_functions_1.add)(1, 1, func);
...
test("test de la fonction forEach", () => {
    ...
    (0, mock_functions_1.forEach)([0, 1], monMock);
