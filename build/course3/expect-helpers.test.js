...
test("Tester qu'un objet contient a minima des propriétés", () => {
    expect({ x: 1, y: 2, c: 3 }).toEqual(expect.objectContaining({
        x: expect.any(Number),
        y: expect.any(Number),
    }));
});
...
test("Tester qu'un objet contient a minima des propriétés", () => {
    expect({ x: 1, y: 2, z: 3 }).toEqual(expect.not.objectContaining({
        a: expect.any(Number),
        b: expect.any(Number),
    }));
});
...
test("ne contient pas la chaîne spécifiée", () => {
    expect("Un Elephant c'est cool").toEqual(expect.stringMatching(/[éÉeE]l[eé]phant/));
});
